...
spawn_temps = lapply(months, function(m){
  ...
  ans$spawn_start = m
#  browser()
  ans2 = aggregate(value.SAC_predicted ~ year, tmp, function(x) sum(x >= 53, na.rm = TRUE))
  ans3 = aggregate(value.SAC_predicted ~ year, tmp, function(x) sum(x >= 56, na.rm = TRUE))
  ans$ndays_over53F = ans2$value.SAC_predicted
  ans$ndays_over56F = ans3$value.SAC_predicted
  ans
})
...
spawn_temps = merge(spawn_temps, surv_eq[c("Temp F", "Total S (IOS)", "Total S (Martin)")],
                    by.x = "value.SAC_predicted", by.y = "Temp F", all.x = TRUE)

i = is.na(spawn_temps$`Total S (IOS)`)

spawn_temps$`Total S (IOS)`[i] = 1
